...
# done
start_time=$(date +%s)
for move_coeff in 1.5; do
    for collision_coeff in -0.01 -0.05 -0.1 -0.2 -0.5; do
        for collision_exp_coeff in 0.2 0.5 1 2; do
            for dist_margin in 2; do
                for survive_rew in 0.1; do
                    python3 -m python.run_vision_ppo --render 0 --train 1 \
                        --move_coeff $move_coeff --collision_coeff $collision_coeff \
                        --collision_exp_coeff $collision_exp_coeff --dist_margin $dist_margin \
                        --survive_rew $survive_rew
                    # --check True
                done
            done
        done
